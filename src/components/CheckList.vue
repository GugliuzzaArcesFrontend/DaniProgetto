<template>
    <div class="card card-outline card-info"
        :class="[{ 'collapsed-card': isCollapsed, 'maximized-card': isMaximized }, sizeClass]">
        <div class="card-header">
            ...
            <div class="card-tools d-flex">
                <ul v-show="totalPages > 1 && !isCollapsed" class="pagination pagination-sm">
                    <li v-if="totalPages > 1 && currentPage > 1" class="page-item">
                    ...
                    <li v-for="page in totalPages" class="page-item">
                        <Void v-if="page > currentPage - 3 && page < currentPage + 3">
                            <a v-if="page != currentPage" @click="changePage(page)" class="page-link">{{ page }}</a>
                    ...
                    <li v-if="currentPage < totalPages" class="page-item">
                        <a @click="changePage(totalPages)" class="page-link">»»</a>
                    </li>
                </ul>
            ...
            <ul class="todo-list">
                <li v-for="(item, i) in paginatedList" :key="i" :id="'todo' + item.id" :class="{ 'done': item.status }">
                    ...
                    <div class="icheck-primary d-inline ml-2">
                        <input type="checkbox" v-model="item.status" :name="item.todo" :value="item.todo"
        ...
        <div v-if="showModal" @click.self="showModal = false" class="modal fade show d-block" id="addItemModal"
            tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" @keyup.enter="addItem">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addItemModalLabel">{{ editingItem ? 'Modifica mansione' :
                            'Nuova mansione' }}</h5>
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="taskName" v-model="newItem.task">
                        </div>

                        <div class="form-group">
                            <label for="dueDate">Scadenza</label>
                            <input type="date" class="form-control" name="dueDate" v-model="newItem.dueDate">
                        </div>
                    </div>

                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary"
                            @click="showModal = false, resetNewItem">Chiudi</button>
...
export default {
    ...
    computed: {
        itemsPage() { return this.isMaximized ? 16 : 4 },
        localList() { return [...this.list.sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate))] },
        totalPages() { return Math.ceil(this.localList.length / this.itemsPage); },
        paginatedList() {
